...
app.post('/api/admin/actualizar', authenticate, requireAdmin, (req: Request, res: Response) => {
    ...
    setImmediate(async () => {
        ...
        try {
            ...
            execSync('npm install', { cwd: path.join(projectRoot, 'backend'), stdio: 'pipe', timeout: 180000 });
            console.log('[UPDATE] npm install frontend...');
            execSync('npm install', { cwd: path.join(projectRoot, 'frontend'), stdio: 'pipe', timeout: 180000 });
            console.log('[UPDATE] npm run build...');
            execSync('npm run build', { cwd: path.join(projectRoot, 'frontend'), stdio: 'pipe', timeout: 120000 });
